...
class HrLeave(models.Model):
        ...
        if 'state' in vals:
            for leave in self:
                ...
                if new_state == 'validate':
                    ...
                    if not leave.is_auto_permission:
    ...
    def _recompute_attendance_for_auto_permission(self):
        """After HR refuses an auto-permission leave, rerun deduction logic
        on the linked attendance so permission_overflow_minutes stays accurate.
        The auto-permission leave is already in 'refuse' state at this point,
        so _clear_stale_auto_permission_leave won't find it, and a fresh
        auto-permission leave (or overflow) will be re-evaluated correctly."""
        self.ensure_one()
        if not self.date_from:
            return
        local_date = self.date_from.date()
        Attendance = self.env['hr.attendance'].sudo()
        attendances = Attendance.search([
            ('employee_id', '=', self.employee_id.id),
            ('check_in', '>=', datetime.combine(local_date, time(0, 0, 0))),
            ('check_in', '<=', datetime.combine(local_date, time(23, 59, 59))),
        ])
        for att in attendances:
            att._apply_permission_deduction()
    ...
    def _check_auto_permission_lock(self):
        """
        Blocks non-HR users from cancelling, resetting, or deleting a leave
        record that was automatically generated by the late-arrival deduction
        system (is_auto_permission=True).

        HR users (hr.group_hr_user) are always allowed through — they may need
        to refuse an auto-deduction when a late detection was incorrect.
        System-level calls (e.g. _clear_stale_auto_permission_leave) run via
        sudo() as the OdooBot/admin user and are therefore also unaffected.
        """
        if self.env.user.has_group('hr.group_hr_user'):
            return
        for leave in self:
            if leave.is_auto_permission:
                raise UserError(_(
                    'Nice Try!! but this permission deduction was created automatically because '
                    'of a late check-in and cannot be cancelled or modified.\n\n'
                    'If you believe this is an error, please contact HR.'
                ))

    def action_draft(self):
        """Override to block employees from resetting auto-permission leaves
        to draft (which would restore the deducted hours)."""
        self._check_auto_permission_lock()
        return super().action_draft()

    def action_refuse(self):
        """Override to block employees from refusing auto-permission leaves
        (which would restore the deducted hours)."""
        self._check_auto_permission_lock()
        return super().action_refuse()

    def action_cancel(self):
        """Override to block the 'Cancel Time Off' wizard from opening for
        auto-permission leaves. The button calls action_cancel() which opens
        hr.holidays.cancel.leave — stopping it here gives a clear error before
        any dialog appears."""
        self._check_auto_permission_lock()
        return super().action_cancel()

    def _action_user_cancel(self, reason=None):
        """Safety-net override: _action_user_cancel() is called by the
        hr.holidays.cancel.leave wizard after action_cancel() opens it.
        We block here too in case the wizard is reached via any other path."""
        self._check_auto_permission_lock()
